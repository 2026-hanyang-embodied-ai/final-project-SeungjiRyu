...
class LaneletInstance : public MapInstance {
 public:
  ...
  bool processOnlyCenterline(const OrientedRect& bbox, const ParametrizationType& paramType, int32_t nPoints,
...
template <typename T>
bool processOnlyLaneletInstances(std::map<Id, std::shared_ptr<T>>& featMap, const OrientedRect& bbox,
                      const ParametrizationType& paramType, int32_t nPoints, double pitch = 0, double roll = 0) {
  bool allValid = true;
  for (auto& feat : featMap) {
    // // Center Line, Left boundary, Right boundary
    // if (!feat.second->process(bbox, paramType, nPoints, pitch, roll)) {
    //   allValid = false;
    // }
    // Center line ONLY
    if (!feat.second->processOnlyCenterline(bbox, paramType, nPoints, pitch, roll)) {
      allValid = false;
    }
  }
  return allValid;
...
template <typename T>
bool processOnlyLaneletInstances(std::vector<std::shared_ptr<T>>& featVec, const OrientedRect& bbox,
                      const ParametrizationType& paramType, int32_t nPoints, double pitch = 0, double roll = 0) {
  bool allValid = true;
  for (auto& feat : featVec) {
    // // Center line, Left boundary, Right boundary
    ...
    // }
    // Center line ONLY
    if (!feat->processOnlyCenterline(bbox, paramType, nPoints, pitch, roll)) {
      allValid = false;
    }
  }
  return allValid;
