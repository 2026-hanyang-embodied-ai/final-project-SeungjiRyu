...
'''
  @file         pluto_model.py
  @brief        Pluto planning model (plain nn.Module, no nuPlan dependency)
...
class PlutoPlanningModel(nn.Module):
    def __init__(
        self,
        dim=128,
        state_channel=6,
        ...
        self.map_encoder = MapEncoder(dim=dim, polygon_channel=polygon_channel, use_lane_boundary=False)
        ...
        self.agent_predictor = AgentPredictor(dim=dim, future_steps=80)
        ...
        if use_hidden_proj:
            self.hidden_proj = nn.Sequential(nn.Linear(dim, dim), nn.ReLU(), nn.Linear(dim, dim))
    ...
    def forward(self, data):
        ...
        if self.ref_free_traj:
            ref_free_traj_angle = torch.arctan2(ref_free_traj[..., 3], ref_free_traj[..., 2])
            out["output_ref_free_trajectory"] = torch.cat(
        ...
        if trajectory is not None:
            ...
            out_trajectory = torch.cat([trajectory[..., :2], angle.unsqueeze(-1)], dim=-1)
            bs2, R, M, T, _ = out_trajectory.shape
            flattened_probability = probability.reshape(bs2, R * M)
            best_trajectory = out_trajectory.reshape(bs2, R * M, T, -1)[
                torch.arange(bs2), flattened_probability.argmax(-1)
            ]
            out["output_trajectory"] = best_trajectory
            out["candidate_trajectories"] = out_trajectory
        else:
            out["output_trajectory"] = out.get("output_ref_free_trajectory")
            out["candidate_trajectories"] = torch.zeros(bs, 0, 0, self.future_steps, 3)
